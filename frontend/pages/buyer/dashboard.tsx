...
export default function BuyerDashboard() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        console.warn("Failed to fetch buyer metrics; default zeros.");
        if (m) {
          setMetrics({
            totalSalesToday: 0,
            openOrders: 0,
  ...
  const fallbackChart: ChartPoint[] = Array(5)
    ...
    .map((_, i) => ({
      ...
      value: [m.totalSalesToday, m.openOrders, m.pendingEscrow, m.availableProducts, m.activeDeals][i] as number,
    }));
            ...
            {METRICS.map((mt) => {
              ...
              return (
                <p key={mt.key} className="mb-1">
                  “{mt.label}”: <span className={mt.color}>{val}</span>
                </p>
              );
            })}
            {analysisText && (
