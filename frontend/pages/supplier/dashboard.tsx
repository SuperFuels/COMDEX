...
export default function SupplierDashboard() {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/terminal/query`, {
        method: "POST",
  ...
  const [dividerX, setDividerX] = useState(300) // px from left

  const onDrag: DraggableEventHandler = (_, data) => {
    if (!containerRef.current) return
    const rect = containerRef.current.getBoundingClientRect()
    const x = Math.min(rect.width - 100, Math.max(100, data.x))
    setDividerX(x)
  }
  ...
  return (
    <div className="bg-bg-page min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-[calc(100%-40px)] mx-auto px-2">
        <div
          ref={containerRef}
          className="relative h-[calc(100vh-4rem-4rem)]"
        >
          {/* Left Pane */}
          <div
            style={{ width: dividerX }}
            className="absolute left-0 top-0 bottom-0 overflow-auto pr-2 font-mono text-text dark:text-text-secondary text-sm"
          >
            <p className="mb-2">
              Hello, Supplier — welcome to Central Command.
            </p>
            {METRICS.map((mt) => (
          ...
          <DraggableCore onDrag={onDrag}>
            <div
              className="absolute top-0 bottom-0 bg-border-light dark:bg-gray-700 cursor-col-resize"
              style={{ left: dividerX - 2, width: 4 }}
            />
          </DraggableCore>
          ...
          <div
            ...
            className="absolute right-0 top-0 bottom-0 overflow-auto pl-2"
          >
            {searchResults ? (
              ...
              <div className="h-full flex items-center justify-center text-text-secondary">
                <p>
                  Select a tab or ask a question to see visual output here.
                </p>
              </div>
            )}
          </div>
