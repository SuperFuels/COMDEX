...
export default function SupplierDashboard() {
  ...
  const m = metrics;
  // fallback chart: hourly ticks for last 5 hours of any numeric
  const fallbackChart: ChartPoint[] = Array(5)
    .fill(0)
    .map((_, i) => ({
      time: Math.floor(Date.now() / 1000) - (4 - i) * 3600,
      value: [m.totalSalesToday, m.activeListings, m.openOrders, m.proceeds30d, m.feedbackRating][i] as number,
    }));
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      ...
      {/* Footer: fixed, width clamped to dividerX - 20px */}
      <footer className="fixed bottom-0 left-0 w-full bg-white border-t py-4">
        <div
          className="mx-auto px-2 flex"
          style={{
            maxWidth: (dividerX ?? 0) - 20,
            width: "100%",
          }}
        >
