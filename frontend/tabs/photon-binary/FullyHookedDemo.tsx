...
export default function FullyHookedDemo() {
  ...
  const startBridge = useCallback(() => {
    ...
    schedule.forEach((ms, idx) => {
      timersRef.current.push(window.setTimeout(() => setActiveParagraphIndex(idx), ms));
    });
  }, [reset]);
      ...
      <div className="max-w-6xl mx-auto px-6 py-24 space-y-24">
        ...
        <header className="text-center space-y-6">
          <motion.h1 
            ...
            className="text-7xl md:text-9xl font-bold tracking-tight italic"
          >
            Photon Bridge
          </motion.h1>
          <p className="text-2xl text-gray-400 font-light max-w-2xl mx-auto leading-relaxed">
            Converting static binary into <span className="text-black font-medium">coherent symbolic intent.</span>
          </p>
        </header>

        {/* Main Demo UI Card */}
        <section className="bg-gray-50 rounded-[3rem] p-4 md:p-12 border border-gray-100 shadow-sm overflow-hidden">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-end px-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-bold italic">{stage === 'binary' ? 'Static Binary' : 'Photon Capsule'}</p>
                </div>
                <div className="text-right space-y-1">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Coherence</p>
                  <p className="text-xl font-mono text-blue-600 font-bold">{(coherence * 100).toFixed(2)}%</p>
                </div>
              </div>
              ...
              <div className="h-[400px] bg-black rounded-[2.5rem] relative flex items-center justify-center overflow-hidden shadow-2xl">
                <AnimatePresence mode="wait">
                  {stage === "binary" ? (
                    ...
                    >
                      {Array(64).fill(0).map((_, i) => <span key={i}>{Math.round(Math.random())}</span>)}
                    </motion.div>
                  ) : (
                    <motion.div 
                      key="wave" 
                    ...
                    >
                      <svg width="80%" height="120" viewBox="0 0 400 100" className="drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]">
                        <motion.path
                      ...
                      <div className="mt-12 grid grid-cols-2 gap-x-12 gap-y-4 text-[10px] font-mono text-gray-500 uppercase tracking-widest">
                        <div>Phase: <span className="text-white">3.141φ</span></div>
                        <div>Hash: <span className="text-white">Hc_{hash}</span></div>
                        <div className="col-span-2 text-center text-blue-400">Status: Entangled Stream Active</div>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button onClick={reset} className="py-5 rounded-full border border-gray-200 text-xs font-bold uppercase tracking-widest hover:bg-white transition-all">Reset State</button>
                <button onClick={startBridge} className="py-5 rounded-full bg-blue-600 text-white text-xs font-bold uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-500/20 transition-all">Trigger Bridge</button>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-center space-y-8 lg:pl-6">
                ...
                <AnimatePresence mode="wait">
                  {activeParagraphIndex === -1 ? (
                    <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                      <h2 className="text-3xl font-bold italic">Awaiting symbolic instruction...</h2>
                      <p className="text-gray-400 leading-relaxed font-light">Initialize the Photonary bridge to see how GlyphNet transforms legacy binary data into self-executing photonic logic.</p>
                    </motion.div>
                  ) : (
                    <motion.div 
                      ...
                      className="space-y-6"
                    >
                      <h2 className="text-3xl font-bold italic text-blue-600">{PARAGRAPHS[activeParagraphIndex].title}</h2>
              ...
              <div className="flex gap-2">
                {PARAGRAPHS.map((_, i) => (
                  <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-500 ${activeParagraphIndex >= i ? 'bg-blue-600' : 'bg-gray-200'}`} />
                ))}
              </div>
            </div>
          </div>
        ...
        <section className="grid md:grid-cols-3 gap-8">
          <div className="p-10 bg-gray-50 rounded-[3rem] space-y-4 border border-gray-100">
            <p className="text-blue-600 font-bold text-xs uppercase tracking-widest">Performance</p>
            <h3 className="text-2xl font-bold italic">56x Reduction</h3>
            <p className="text-gray-500 text-sm leading-relaxed">By treating light as the message itself rather than a carrier for binary, we eliminate the metadata bloat found in TCP/IP.</p>
          </div>
          <div className="p-10 bg-black text-white rounded-[3rem] space-y-4 shadow-2xl">
            <p className="text-blue-400 font-bold text-xs uppercase tracking-widest">Security</p>
            <h3 className="text-2xl font-bold italic">Collapse Hashes</h3>
            <p className="text-gray-400 text-sm leading-relaxed">Every state transition is cryptographically bound to its physical wave properties, making tampering physically impossible.</p>
          </div>
          <div className="p-10 bg-gray-50 rounded-[3rem] space-y-4 border border-gray-100">
            <p className="text-blue-600 font-bold text-xs uppercase tracking-widest">Hardware</p>
            <h3 className="text-2xl font-bold italic">Edge Native</h3>
            <p className="text-gray-500 text-sm leading-relaxed">Runs on classical silicon today via SQI, transitioning to native photonic fibers in the next iteration.</p>
          </div>
        </section>

        {/* Footer info to ensure scrolling depth */}
        <footer className="pt-24 pb-12 border-t border-gray-100 text-center">
          <p className="text-gray-300 font-mono text-[10px] uppercase tracking-[0.5em]">Tessaris Research Group // SRK-17 Pipeline</p>
        </footer>
      </div>
    </div>
  );
