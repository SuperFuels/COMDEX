...
export default function PhotonBridgeDemo() {
  const [stage, setStage] = useState("binary");
      ...
      <div className="max-w-7xl mx-auto px-8 space-y-16">
          ...
          <div className="grid lg:grid-cols-2 gap-16">
            ...
            <div className="space-y-8">
              ...
              <div className="h-[500px] bg-black rounded-[2.5rem] relative flex items-center justify-center overflow-hidden shadow-2xl group border-[12px] border-gray-900/10">
                <AnimatePresence mode="wait">
                  {stage === "binary" ? (
                    <motion.div 
                      key="bin" initial={{ opacity: 0 }} animate={{ opacity: 0.25 }} exit={{ opacity: 0 }}
                      className="grid grid-cols-8 gap-6 text-blue-500 font-mono text-[10px]"
                    >
                      {Array(64).fill(0).map((_, i) => <span key={i} className="animate-pulse">{Math.round(Math.random())}</span>)}
                    </motion.div>
                  ) : (
                    <motion.div 
                      key="wave" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} 
                      className="w-full flex flex-col items-center"
                    >
                      {/* WAVEFORM DIAGRAM */}
                      
                      <svg width="90%" height="200" viewBox="0 0 400 100" className="drop-shadow-[0_0_25px_rgba(59,130,246,0.6)]">
                        <motion.path
                          ...
                          strokeWidth="3"
                          strokeLinecap="round"
                          animate={{ d: ["M 0 50 C 50 10, 150 10, 200 50 S 350 90, 400 50", "M 0 50 C 50 90, 150 90, 200 50 S 350 10, 400 50"] }}
                      ...
                      <div className="mt-12 text-center space-y-4">
                        <div className="flex gap-8 justify-center">
                          <div className="text-[10px] font-mono text-gray-500 uppercase">Phase: <span className="text-white">3.141φ</span></div>
                          <div className="text-[10px] font-mono text-gray-500 uppercase">Entropy: <span className="text-white">{(1 - coherence).toFixed(4)}</span></div>
                        </div>
                ...
                <div className="absolute bottom-8 flex gap-4">
                  <button onClick={reset} className="px-8 py-3 bg-white/5 hover:bg-white/10 backdrop-blur-xl rounded-full text-[11px] text-white font-bold uppercase tracking-widest border border-white/10 transition-all">
                    Reset Substrate
                  </button>
                  <button onClick={toggleAutoPlay} className={`px-8 py-3 rounded-full text-[11px] text-white font-bold uppercase tracking-widest transition-all shadow-xl ${isAutoPlaying ? 'bg-red-600 shadow-red-500/20' : 'bg-blue-600 shadow-blue-500/40'}`}>
            ...
            <div className="flex flex-col justify-between py-10 min-h-[500px]">
              <div className="space-y-10">
                <AnimatePresence mode="wait">
                  {activeIndex === -1 ? (
                    <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                      ...
                      <button onClick={handleNext} className="group flex items-center gap-4 text-blue-600 font-bold uppercase text-xs tracking-[0.2em] pt-4">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-[10px] font-mono text-gray-400 uppercase tracking-widest">Reference // SRK-SPEC-v1.2</span>
                      </div>
                      
                      <h2 className="text-4xl font-bold italic tracking-tighter text-black">
                        {PARAGRAPHS[activeIndex].title}
                      </h2>
              ...
              <div className="pt-12 space-y-8">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-4">
                    <button onClick={handlePrev} className="w-14 h-14 rounded-full border border-gray-200 flex items-center justify-center hover:bg-white hover:shadow-lg transition-all text-xl">←</button>
                    <button onClick={handleNext} className="w-14 h-14 rounded-full border border-gray-200 flex items-center justify-center hover:bg-white hover:shadow-lg transition-all text-xl">→</button>
                  </div>
                  ...
                  <div className="flex gap-3">
                    {PARAGRAPHS.map((_, i) => (
                      <button 
                        key={i} 
                        onClick={() => goToIndex(i)}
                        className={`h-2 rounded-full transition-all duration-500 ${activeIndex === i ? 'w-12 bg-blue-600' : 'w-2 bg-gray-200 hover:bg-gray-400'}`} 
                      />
                    ))}
                  </div>
                </div>
        ...
        <section className="grid md:grid-cols-3 gap-10">
          <div className="p-12 bg-gray-50 rounded-[3rem] border border-gray-100 hover:border-blue-100 transition-colors group">
            ...
            <h3 className="text-3xl font-bold italic mb-4 group-hover:text-blue-600 transition-colors tracking-tighter">56x Metadata Reduction</h3>
            <p className="text-gray-500 text-lg leading-relaxed font-light">
              By utilizing the WaveCapsule protocol, GlyphNet bypasses standard IP overhead, enabling zero-latency symbolic exchange.
            </p>
          </div>
          <div className="p-12 bg-black text-white rounded-[3rem] shadow-2xl relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform">
              <svg width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            </div>
            ...
            <p className="text-gray-400 text-lg leading-relaxed font-light">
              Quantum Key Distribution (QKD) ensures that any observation forces a wave collapse, making tampering physically evident.
            </p>
          </div>
          <div className="p-12 bg-gray-50 rounded-[3rem] border border-gray-100 hover:border-blue-100 transition-colors group">
            <p className="text-blue-600 font-bold text-xs uppercase tracking-widest mb-6">Execution</p>
            <h3 className="text-3xl font-bold italic mb-4 group-hover:text-blue-600 transition-colors tracking-tighter">Lyapunov Stability</h3>
            <p className="text-gray-500 text-lg leading-relaxed font-light">
              The Resonance Ledger monitors temporal coherence, ensuring that only meaning-stable packets reach the execution layer.
            </p>
          </div>
        </section>

        <footer className="pt-20 pb-10 border-t border-gray-100 text-center opacity-40">
          <p className="font-mono text-[10px] uppercase tracking-[0.6em]">Tessaris Research Group // SRK-10 Photon Bridge Substrate // 2026</p>
        </footer>
      </div>
    </div>
  );
