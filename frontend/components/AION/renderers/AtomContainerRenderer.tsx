 ...
 */
const AtomContainerRenderer: React.FC<AtomContainerRendererProps> = ({ position, container }) => {
  const atoms = container.atoms ?? [];

  return (
    <group position={position as THREE.Vector3 | [number, number, number]}>
      {atoms.map((atom, i) => (
        <AtomContainer
          ...
          atom={{
            ...
            viz: {
              ...
              position: [
                position[0] + Math.cos((i / Math.max(atoms.length, 1)) * Math.PI * 2) * 2.2,
                position[1] + 0.4 * ((i % 2) ? 1 : -1),
                position[2] + Math.sin((i / Math.max(atoms.length, 1)) * Math.PI * 2) * 2.2,
              ],
            },
          }}
        />
