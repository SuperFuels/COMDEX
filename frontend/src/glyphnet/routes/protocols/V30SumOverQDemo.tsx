...
function svgQGraph(opts: {
  ...
  // 1) Q-density bar (q/n)
  // 2) Work bar (measured steps vs ~k*q bound)
  ...
  // only show "work" if steps is real and bound is > 0
  ...
  // IMPORTANT: never force a nonzero width when the fraction is 0 (prevents "always visible" bar)
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} role="img" aria-label="Q scaling graph">
      ...
      <text x={x1} y={qBandY + bandH + 14} fontSize={11} fill="#6b7280" textAnchor="end">
        n={n.toLocaleString()} · |Q|={q.toLocaleString()} · hits={hitsInQ.toLocaleString()}
      </text>
...
export const V30SumOverQDemo: React.FC = () => {
  ...
  function applyPreset(which: "balanced" | "stress" | "tiny" | "bigQ" | "smallQ") {
    if (which === "balanced") {
      ...
      setQ((prev) => clamp(prev * 2, 1, 4096));
    } else {
      setQ((prev) => clamp(Math.floor(prev / 2), 1, 4096));
    }
  }
  ...
  // best-effort “work” counter: ONLY real if backend emitted something finite
  const measuredStepsRaw =
    inv?.work_steps ?? inv?.steps ?? inv?.sum_steps ?? inv?.q_steps ?? inv?.tracked_steps;
  const measuredSteps = asFinite(measuredStepsRaw, NaN);
  const hasSteps = Number.isFinite(measuredSteps);

  const bound = hasSteps ? Math.ceil(4 * Math.max(1, q_size)) : 0; // only meaningful if steps exist

  // seller framing
  const fanoutBytes = Number(wire_total_bytes || 0) * Math.max(0, Number(audience || 0));
  const estRevenue = Math.max(0, Number(audience || 0)) * Math.max(0, Number(price || 0));

  const curl = useMemo(() => {
    const body = JSON.stringify({ seed, n, turns, muts, q });
      ...
      >
        ...
        <div style={{ ...cardStyle() }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
            ...
            <div style={{ fontSize: 10, color: "#6b7280", fontWeight: 900 }}>subset aggregate framing</div>
          </div>
          <div style={{ marginTop: 8, ...miniText() }}>
            Imagine <b>Q</b> is “your buyers of interest” (VIPs / fraud watchlist / cohort) and you want the SUM over their counters
            without tracking everyone.
          </div>
          ...
          <div style={{ marginTop: 12, borderRadius: 14, border: "1px solid #e5e7eb", background: "#f9fafb", padding: 10 }}>
            ...
            <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr", gap: 6, fontSize: 11, color: "#374151" }}>
              <div>
                Update bytes (one consumer): <b style={{ color: "#111827" }}>{bytes(Number(wire_total_bytes || 0))}</b>
              </div>
              <div>
              ...
              <div style={{ borderRadius: 14, border: "1px solid #e5e7eb", background: "#fff", padding: 10 }}>
                ...
                <div style={{ marginTop: 6, fontSize: 11, color: "#374151" }}>
                  ...
                  <span style={{ color: "#6b7280" }}>
                    (hit-rate {q_size ? ((hits_in_Q / q_size) * 100).toFixed(2) : "0.00"}%)
                  </span>
                </div>
              </div>
              <div style={{ borderRadius: 14, border: "1px solid #e5e7eb", background: "#fff", padding: 10 }}>
          ...
          {out ? (
            <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div style={{ borderRadius: 16, border: "1px solid #e5e7eb", padding: 12 }}>
                ...
                <div style={{ marginTop: 8, fontSize: 11, color: "#374151", lineHeight: 1.6 }}>
                  <div>
                    ops_total: <b style={{ color: "#111827" }}>{b?.ops_total ?? "—"}</b>
                  </div>
                  <div>
                    q_size: <b style={{ color: "#111827" }}>{b?.q_size ?? "—"}</b> &nbsp; hits_in_Q:{" "}
                    <b style={{ color: "#111827" }}>{b?.hits_in_Q ?? "—"}</b>
                  </div>
                  <div>
                    delta_bytes_total:{" "}
          ...
          <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
            <div style={{ borderRadius: 14, border: "1px solid #e5e7eb", background: "#f9fafb", padding: 10 }}>
              ...
              <div style={{ marginTop: 6, ...miniText() }}>
                You don’t have to track all <code>n</code> indices. Track only <b>Q</b> and maintain the SUM exactly.
              </div>
            ...
            <div style={{ borderRadius: 14, border: "1px solid #e5e7eb", background: "#f9fafb", padding: 10 }}>
              ...
              <div style={{ marginTop: 8, fontSize: 11, color: "#374151" }}>
                This is the “pay only for what you query” primitive.
              </div>
            </div>
