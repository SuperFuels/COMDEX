      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#6b7280", marginTop: 3, maxWidth: 860 }}>
            Track only indices in <b>Q</b> on a delta stream. Still produce the exact same Projection(Q) as full replay — and lock it with
            a receipt.
          </div>
        </div>
      ...
      >
        ...
        <div style={{ ...cardStyle() }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
            ...
            <div style={{ fontSize: 10, color: "#6b7280", fontWeight: 900 }}>subset reads on a stream</div>
          </div>
          <div style={{ marginTop: 8, ...miniText() }}>
            Imagine <b>Q</b> is your “customers of interest” (VIPs / fraud watchlist / cohort). You want only their state, not the whole
            world, and you want it verifiable.
          </div>
          ...
          <div style={{ marginTop: 10, borderTop: "1px solid #f3f4f6", paddingTop: 10 }}>
            ...
            <div style={{ marginTop: 6, fontSize: 11, color: "#6b7280" }}>
              <code>POST /api/wirepack/v29/run</code>
            </div>
          </div>
        </div>
            ...
            <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <div style={{ borderRadius: 16, border: "1px solid #e5e7eb", padding: 12 }}>
                ...
                <div style={{ marginTop: 8, fontSize: 11, color: "#374151", lineHeight: 1.6 }}>
                  ...
                  <div>
                    template_bytes:{" "}
                    <b style={{ color: "#111827" }}>{bytes(Number(b?.template_bytes ?? 0))}</b>{" "}
                    <span style={{ color: "#6b7280" }}>{b?.template_bytes != null ? `(${b.template_bytes} B)` : ""}</span>
                  </div>
                  <div>
                    delta_bytes_total:{" "}
                    <b style={{ color: "#111827" }}>{bytes(Number(b?.delta_bytes_total ?? 0))}</b>{" "}
                    <span style={{ color: "#6b7280" }}>{b?.delta_bytes_total != null ? `(${b.delta_bytes_total} B)` : ""}</span>
                  </div>
                  <div>
                    wire_total_bytes:{" "}
                    <b style={{ color: "#111827" }}>{bytes(Number(b?.wire_total_bytes ?? 0))}</b>{" "}
          ...
          <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
            <div style={{ borderRadius: 14, border: "1px solid #e5e7eb", background: "#f9fafb", padding: 10 }}>
              ...
              <div style={{ marginTop: 6, ...miniText() }}>
                Projection(Q) means: “give me the exact values at indices in Q.” You can do it while ingesting a delta stream, without
                materializing the full state.
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#374151" }}>
                Stored: only tracked indices + last-write-wins values
              </div>
            </div>
