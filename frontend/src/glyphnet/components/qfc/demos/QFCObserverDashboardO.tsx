...
const MODES = {
  O2_EQUILIBRIUM: { id: 0, label: "O2: Equilibrium", fidelity: 0.911, mi: 0.671, drift: 8.096e-5, status: "STABLE", color: "#00f2ff" },
  O3_OVERCOUPLED: { id: 1, label: "O3: Overcoupled", fidelity: 0.88, mi: 0.75, drift: -1.518e-4, status: "OSCILLATORY", color: "#facc15" },
  O4_UNLOCKED: { id: 2, label: "O4: Unlocked", fidelity: 0.847, mi: 0.997, drift: 0.005, status: "DECOHERENT", color: "#f87171" },
  O4A_SERVO: { id: 3, label: "O4a: Phase Servo", fidelity: 0.922, mi: 0.997, drift: 1e-6, status: "METASTABLE", color: "#38bdf8" },
  O8_PREDICT: { id: 4, label: "O8: Predict Horizon", fidelity: 0.911, mi: 0.671, drift: 1.065e-3, status: "PREDICTIVE", color: "#c084fc", corr: 0.999 },
  O10_DIVERGENT: { id: 5, label: "O10: Divergent Loop", fidelity: 0.72, mi: 0.45, drift: -2.66e-4, status: "DIVERGENT", color: "#ef4444", corr: 0.98 },
} as const;
...
const VolumetricField = ({ isObserver, modeData, time }: { isObserver: boolean; modeData: any; time: number }) => {
  ...
  return (
    <points ref={pointsRef} position={[0, isObserver ? 4 : -4, 0]} rotation={[-Math.PI / 2, 0, 0]}>
...
const CouplingLattice = ({ modeData, time }: { modeData: any; time: number }) => {
  const count = 12;
  const positions = useMemo(() => Array.from({ length: count }, (_, i) => (i - count / 2) * 2.5), []);
...
export default function QFCObserverDashboardO({ frame }: { frame?: any }) {
  ...
  useFrame((_, dt) => { tRef.current += Math.min(dt, 1/30); });
  ...
  return (
    <group>
      ...
      <Float speed={1.5} floatIntensity={0.2}>
        <Text position={[-10, -5, 0]} fontSize={0.6} color="#00f2ff" rotation={[0, Math.PI / 4, 0]}>Ψ_SYS</Text>
        <Text position={[-10, 5, 0]} fontSize={0.6} color="#8b5cf6" rotation={[0, Math.PI / 4, 0]}>Ψ_OBS</Text>
      </Float>
      ...
      <Html fullscreen transform={false}>
        <div className="pointer-events-none w-full h-full font-mono text-slate-200 p-8">
          ...
          <div className="absolute top-8 right-8 w-80 p-6 bg-black/60 backdrop-blur-xl border border-white/10 rounded-lg pointer-events-auto">
            <h2 className="text-cyan-400 text-xs tracking-[0.3em] font-bold border-b border-white/10 pb-2 mb-4">OBSERVER_IDENTITY_V7</h2>
            
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-[10px] mb-1">
                  <span className="text-slate-500 uppercase">Mutual Information</span>
                  <span className="text-white">{current.mi.toFixed(3)} bits</span>
                </div>
                <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-cyan-500 transition-all duration-700" style={{ width: `${current.mi * 100}%` }} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div className="bg-white/5 p-2 rounded">
                  ...
                  <div className="text-purple-400 font-bold">{(current.fidelity * 100).toFixed(1)}%</div>
                </div>
                <div className="bg-white/5 p-2 rounded">
                  <div className="text-slate-500">DRIFT</div>
                  <div className="text-red-400 font-bold">{current.drift.toExponential(2)}</div>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-white/5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] uppercase tracking-widest" style={{ color: current.color }}>{current.status}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="absolute bottom-8 left-8 flex flex-col gap-1 pointer-events-auto">
             <span className="text-[8px] text-slate-500 mb-2 tracking-[0.4em]">SELECT_MODE_O_SERIES</span>
                ...
                {(Object.keys(MODES) as ModeKey[]).map((m) => (
                  <button
                    ...
                    className={`px-3 py-1 text-[9px] border transition-all ${
                      mode === m ? "bg-white text-black border-white" : "border-white/20 text-slate-400 hover:border-white/50"
                    }`}
                  >
                    {m.split('_')[0]}
                  </button>
