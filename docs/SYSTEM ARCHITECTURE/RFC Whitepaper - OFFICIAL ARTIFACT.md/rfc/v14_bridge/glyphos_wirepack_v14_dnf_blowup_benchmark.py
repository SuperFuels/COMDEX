...
def bench(max_n: int = 18):
    ...
    for n in range(1, max_n + 1):
        ...
        terms = dnf_terms(f)
        dnfj = json.dumps({"dnf": terms}, separators=(",", ":"), ensure_ascii=False).encode(
            "utf-8"
        )
    ...
    for r in rows:
        print(
            f"{r['n']:2d} | {r['terms']:10d} | {r['canon_raw']:9d} | {r['canon_gz']:8d} | {r['dnf_raw']:7d} | {r['dnf_gz']:6d} | {r['gz_ratio']:19.2f}"
        )
