...
def write_run_artifacts(*, base_dir: Path, test_id: str, run_hash: str, run: Mapping[str, Any]) -> Path:
    ...
    S_series = run.get("S_series", [])
    R_series = run.get("R_series", [])
    g_series = run.get("g_series", [])
    norm_series = run.get("norm_series", [])

    rows = ["t,S,R,gain,norm"]
    n = min(len(S_series), len(R_series), len(g_series), len(norm_series))
    for i in range(n):
        rows.append(f"{i},{S_series[i]},{R_series[i]},{g_series[i]},{norm_series[i]}")
