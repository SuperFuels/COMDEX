...
app.get("/health", (_req, res) => {
    res.json({
        ...
        maxRfIngressBytes: MAX_RF_INGRESS_BYTES, // ← add this
    });
});
...
app.get(["/api/container/:id", "/api/containers/:id"], (req, res) => {
    ...
    // minimal shape so the landing page stops complaining
    res.json({
        ok: true,
        id,
        status: "local",
        ...
        topic: `ucs://local/${id}`,
        graph: "personal",
        ws: `/ws/ghx/${encodeURIComponent(id)}`, // what the page will try to open
    });
});
...
// TX (token required)
app.post("/bridge/tx", requireBridgeToken, (req, res) => {
...
wss.on("connection", (ws, req) => {
    try {
        ...
        ws.on("message", (data) => {
            ...
            // 1) local WS fanout (always echo to room)
            broadcast(key, { type: "glyphnet_capsule", envelope });
            // 2) RF bridge for WS-origin voice payloads — size-guarded
            const cap = envelope.capsule || {};
            // helper to notify sender on oversize
...
const wssRF = new WebSocketServer({ server, path: "/ws/rflink" });
...
wssRF.on("connection", (ws, req) => {
    ...
    try {
        ...
        ok = (url.searchParams.get("token") || "") === BRIDGE_TOKEN;
    }
    catch { }
    if (!ok) {
        try {
    ...
    if (activeBridge && activeBridge.readyState === WebSocket.OPEN) {
        try {
            activeBridge.close(1000, "superseded");
        }
        catch { }
    }
    activeBridge = ws;
    console.log("[rf-link] bridge connected");
    try {
        ws.send(JSON.stringify({ type: "hello", mtu: ACTIVE.MTU, rate_hz: ACTIVE.RATE_HZ }));
    }
    catch { }
    ws.on("message", (data) => {
        // expect: {"type":"rx","topic":"kg:recipient","bytes_b64":"...","codec"?}
        let msg;
        try {
        ...
        if (msg?.type === "rx" && typeof msg.topic === "string" && typeof msg.bytes_b64 === "string") {
            ...
            const key = msg.topic; // already "graph:recipient"
            const envelope = {
                capsule: { glyphs: ["(rf)"], rf_bytes_len: bytes.length },
                meta: { graph: key.split(":")[0] || "personal" },
                ts: now(),
                id: msgId(),
            };
            broadcast(key, { type: "glyphnet_capsule", envelope });
    ...
    ws.on("close", () => {
        if (activeBridge === ws)
            activeBridge = null;
        console.log("[rf-link] bridge disconnected");
    });
    ws.on("error", () => {
        if (activeBridge === ws)
            activeBridge = null;
...
// push any queued frames immediately
    ...
    while (rfOutbox.length) {
        ...
        try {
            activeBridge.send(JSON.stringify({
                type: "tx",
                bytes_b64: Buffer.from(frame).toString("base64"),
            }));
        }
        catch {
            // if send fails, stop and let the ticker try again next tick
            rfOutbox.unshift(frame);
            break;
        }
    }
}
// Nudge the bridge each RF tick, after we place frames into rfOutbox
const _oldTicker = rfTicker;
if (_oldTicker)
    clearInterval(_oldTicker);
const hz = Math.max(1, Math.floor(ACTIVE.RATE_HZ || 10));
const intervalMs = Math.max(1, Math.round(1000 / hz));
rfTicker = setInterval(() => {
    const frame = rfQueue.shift();
...
server.on("upgrade", (req, socket, head) => {
    try {
        const url = new URL(req.url || "", "http://localhost");
        // match /ws/ghx/{name}
        if (url.pathname.startsWith("/ws/ghx/")) {
            wssGHX.handleUpgrade(req, socket, head, (ws) => {
                wssGHX.emit("connection", ws, req);
            });
            return;
        }
        // NOTE: your /ws/glyphnet is already handled by the other WebSocketServer with { path: "/ws/glyphnet" }
    }
    catch {
        socket.destroy();
    }
...
wssGHX.on("connection", (ws, req) => {
    const url = new URL(req.url || "", "http://localhost");
    const container = url.pathname.split("/").pop() || "unknown";
    // simple heartbeat so UI shows connected
    const timer = setInterval(() => {
        try {
            ws.send(JSON.stringify({ type: "ghx/heartbeat", at: Date.now(), container }));
        }
        catch { }
    }, 15000);
