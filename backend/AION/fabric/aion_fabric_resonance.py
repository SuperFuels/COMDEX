...
────────────────────────────────────────────
Receives normalized ψ–κ–T–Φ packets from the Photon Ingest bridge,
accumulates them into a transient fusion buffer, and produces a
live fusion tensor. This layer now provides cross-process visibility
...
def resonance_ingest(packet: Dict, timestamp: Optional[float] = None):
    """
    Accepts an incoming ψ–κ–T–Φ cognitive packet, validates it,
    and stores it in the AION Morphic buffer.
    """
    ...
    logger.info(
        ...
        f"T={packet.get('T')} Φ={packet.get('Φ')} (S={packet.get('stability')})"
    )
    return True
...
# ────────────────────────────────────────────────
# Phase II — Tensor Fusion Logic
# ────────────────────────────────────────────────
def fuse_resonance_window(window_size: int = 10) -> Optional[Dict]:
    """
    Compute a ψ–κ–T fusion tensor from the most recent N packets.
    Performs temporal averaging and derives a symbolic stability index.

    Returns a dict like:
      {"ψ̄": <mean>, "κ̄": <mean>, "T̄": <mean>, "Φ̄": <mean>, "σ": <stability>}
    ...
    # Compute stability as 1 - normalized variance
    var_sum = sum(
    ...
    logger.info(f"[AIONFabric] Fused {n} packets → ψ̄={fused['ψ̄']} κ̄={fused['κ̄']} "
                f"T̄={fused['T̄']} σ={fused['σ']}")
...
def get_latest_fusion_tensor() -> Optional[dict]:
    ...
    # Prefer memory if present
    if _latest_fusion_tensor:
        return _latest_fusion_tensor

    # Otherwise, read from shared file
