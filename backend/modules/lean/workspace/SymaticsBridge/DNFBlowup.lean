...
Mathlib-free (core Nat lemmas only).
-/
...
/-- Blowup family:
    F(0) is modeled as a single variable (one term).
    F(n+1) = F(n) ∧ pair(n). -/
...
/-- Tiny arithmetic lemma: 1 + (3 + t) = 4 + t. -/
theorem one_add_three_add (t : Nat) : 1 + (3 + t) = 4 + t := by
  calc
    1 + (3 + t) = (1 + 3) + t := by
      exact (Nat.add_assoc 1 3 t).symm
    _ = 4 + t := by
      rfl

/-- Tiny arithmetic lemma: 1 + t + 3 = t + 4. -/
theorem one_add_t_add_three (t : Nat) : 1 + t + 3 = t + 4 := by
  calc
    1 + t + 3 = 1 + (t + 3) := by
      exact Nat.add_assoc 1 t 3
    _ = 1 + (3 + t) := by
      simp [Nat.add_comm]
    _ = 4 + t := by
      exact one_add_three_add t
    _ = t + 4 := by
      simp [Nat.add_comm]
...
theorem nodes_family_succ (n : Nat) :
  ...
  calc
    ...
    _   = nodes (family n) + 4 := by
            exact one_add_t_add_three (nodes (family n))
...
theorem nodes_family : ∀ n : Nat, nodes (family n) = 1 + 4*n := by
  ...
  | succ n ih =>
      calc
        nodes (family (n + 1))
            = nodes (family n) + 4 := by
                exact nodes_family_succ n
        _   = (1 + 4 * n) + 4 := by
                simp [ih]
        _   = 1 + (4 * n + 4) := by
                exact Nat.add_assoc 1 (4 * n) 4
        _   = 1 + 4 * (n + 1) := by
                simp [Nat.mul_succ]
