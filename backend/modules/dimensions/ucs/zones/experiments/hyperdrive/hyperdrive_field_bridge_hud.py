...
from backend.modules.dimensions.ucs.zones.experiments.hyperdrive.hyperdrive_field_bridge_hud import HyperdriveFieldBridgeHUD


class HyperdriveFieldBridgeClient:
    def __init__(self, uri: str = "ws://raspberrypi.local:8765", token: Optional[str] = None, target_voltage: float = 1.0):
        ...
        self.hud = HyperdriveFieldBridgeHUD(max_points=300, ...) target_voltage=target_voltage)
        self.last_duty = 0.0
    ...
    async def connect(self):
        print(f"🔌 Connecting to Hyperdrive FieldBridge at {self.uri}...")
...
if __name__ == "__main__":
    ...
    client = HyperdriveFieldBridgeClient(uri=args.uri, token=args.token, target_voltage=args.target_voltage)
