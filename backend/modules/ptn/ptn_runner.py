...
────────────────────────────────────────────
Executes .ptn Photon Pages through Photon Language engine.
...
def run_photon_page(path: str, *, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Build equivalent Photon capsule
    capsule = {
        "name": page.name,
        "engine": "symatics",
        "glyphs": [
    ...
    logger.info(f"[PTNRunner] Executing PhotonPage: {page.name}")
