import hashlib
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
LOCKDIR = ROOT / "backend" / "tests" / "locks"

SCRIPT = ROOT / "backend" / "tests" / "glyphos_wirepack_v27_state_space_sparse_update_benchmark.py"
LEAN = ROOT / "backend" / "modules" / "lean" / "workspace" / "SymaticsBridge" / "V27_StateSpaceSparseUpdate.lean"

OUT = LOCKDIR / "v27_state_space_sparse_update_out.txt"
LOCK = LOCKDIR / "v27_state_space_sparse_update_lock.sha256"


def sha256_file(p: Path) -> str:
    h = hashlib.sha256()
    with p.open("rb") as f:
        for chunk in iter(lambda: f.read(1 << 20), b""):
            h.update(chunk)
    return h.hexdigest()


def parse_lock_lines(text: str) -> dict:
    out = {}
    for line in text.strip().splitlines():
        if not line.strip():
            continue
        parts = line.split()
        if len(parts) < 2:
            continue
        h, p = parts[0], parts[1]
        out[p] = h
        out[Path(p).name] = h  # allow basename lookup
    return out


def test_v27_lock_files_exist():
    assert LEAN.exists(), f"missing: {LEAN}"
    assert SCRIPT.exists(), f"missing: {SCRIPT}"
    assert OUT.exists(), f"missing: {OUT} (run benchmark + tee first)"
    assert LOCK.exists(), f"missing: {LOCK} (run sha256sum lock step)"


def test_v27_locks_match():
    lock_map = parse_lock_lines(LOCK.read_text(encoding="utf-8"))

    # Accept either (LEAN + OUT) (v24-style) OR (SCRIPT + OUT) (v26-style)
    lean_keys = [str(LEAN), str(LEAN.relative_to(ROOT)), LEAN.name]
    script_keys = [str(SCRIPT), str(SCRIPT.relative_to(ROOT)), SCRIPT.name]
    out_keys = [str(OUT), str(OUT.relative_to(ROOT)), OUT.name]

    exp_lean = next((lock_map.get(k) for k in lean_keys if lock_map.get(k)), None)
    exp_script = next((lock_map.get(k) for k in script_keys if lock_map.get(k)), None)
    exp_out = next((lock_map.get(k) for k in out_keys if lock_map.get(k)), None)

    assert exp_out is not None, "lock file must include out"
    assert (exp_lean is not None) or (exp_script is not None), "lock file must include lean or script"

    got_out = sha256_file(OUT)
    assert got_out == exp_out

    if exp_lean is not None:
        got_lean = sha256_file(LEAN)
        assert got_lean == exp_lean
    if exp_script is not None:
        got_script = sha256_file(SCRIPT)
        assert got_script == exp_script