#!/usr/bin/env python3
"""
v45 — Cross-language codec test vectors (Python ↔ Rust)
...
- Lean compile check uses:
    backend/modules/lean/workspace/SymaticsBridge/V45_CrossLangWirePackVectors.lean
- If Lean fails, LEAN_OK=0 and benchmark still succeeds unless REQUIRE_LEAN=1.
- If UPDATE_LOCKS=1:
    - writes backend/tests/locks/v45_crosslang_vectors_out.txt itself (NO tee needed)
    - writes backend/tests/locks/v45_crosslang_vectors_lock.sha256 using sha256(Lean file) and sha256(out file)

Rust:
- Expects Rust binary at:
    tools/glyphos_wirepack_v45_vectors.bin
...
LEAN_REL = "SymaticsBridge/V45_CrossLangWirePackVectors.lean"
LEAN_WS = ROOT / "backend/modules/lean/workspace"
LEAN_FILE = LEAN_WS / LEAN_REL
THIS_FILE = Path(__file__).resolve()

LOCK_DIR = ROOT / "backend/tests/locks"
OUT_FILE = LOCK_DIR / "v45_crosslang_vectors_out.txt"
LOCK_SHA = LOCK_DIR / "v45_crosslang_vectors_lock.sha256"

RUST_BIN = ROOT / "tools/glyphos_wirepack_v45_vectors.bin"
...
def main() -> None:
    ...
    a("=== ✅ Bridge Benchmark v45: Cross-language WirePack codec vectors (Python ↔ Rust) ===")
    a("v45_crosslang_vectors_wirepack")
    a(f"seed={SEED}")
