...
LEAN_FILE = ROOT / "backend/modules/lean/workspace/SymaticsBridge/V38_CanonicalDeltaDeterminismReplay.lean"
...
def lean_check() -> int:
    """
    Compile-check the Lean bridge file using the workspace's lake env.
    This does NOT build the whole SymaticsBridge library, so it won't
    re-fetch Mathlib unless the target file itself imports Mathlib.
    """
    ws = ROOT / "backend/modules/lean/workspace"
    r = subprocess.run(
