...
def load_resonance(path):
    ...
    if isinstance(data, dict):
        ...
        if "averages" in data and isinstance(data["averages"], dict):
            ...
            SQI = avg.get("SQĪ") or avg.get("SQI") or 0.0
            return {"ρ̄": float(ρ), "Ī": float(I), "SQĪ": float(SQI)}
        ...
        SQI = data.get("SQĪ") or data.get("\\u0053\\u0051\\u0049\\u0304") or data.get("SQI") or 0.0
        return {"ρ̄": float(rho), "Ī": float(I), "SQĪ": float(SQI)}
...
def validate_cycle():
    ...
    SQI_diff = round(abs(lex["SQĪ"] - math["SQĪ"]), 3)
