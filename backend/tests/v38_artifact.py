...
def generate_v38_artifact(out_dir: str | None = None) -> Path:
    ...
    for _ in range(10):
        ...
        # execute_qpu_opcode returns List[str]; nabla opcode returns float -> it may come back as float
        nabla_vals.append(float(v))

    beams = getattr(cell, "wave_beams", []) or []
    eids = _collect_eids(beams)

    # --- Lean validation surface (container-style) ---
    lean_container: Dict[str, Any] = {
        ...
        "symbolic_logic": [
            {
                "name": "v38_equiv_demo",
                "symbol": "⟦ Theorem ⟧",
                # “canonical ops” appear here; lean_utils validates presence + normalization, not proof correctness
                "logic": "logic:↔(x,y)",
