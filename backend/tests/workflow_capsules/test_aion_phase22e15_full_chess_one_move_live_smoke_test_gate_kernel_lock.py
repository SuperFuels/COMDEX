from pathlib import Path

from backend.modules.aion_games.full_chess_one_move_live_smoke_test_gate_kernel import (
    run_full_chess_one_move_live_smoke_test_gate_kernel,
)


def test_phase22e15_blocks_by_default_without_token_or_confirmation(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("LICHESS_BOT_TOKEN", raising=False)

    result = run_full_chess_one_move_live_smoke_test_gate_kernel(
        memory_path=tmp_path / "smoke_memory.json",
        preflight_memory_path=tmp_path / "preflight_memory.json",
        gate_memory_path=tmp_path / "gate_memory.json",
        adapter_memory_path=tmp_path / "adapter_memory.json",
        loop_memory_path=tmp_path / "loop_memory.json",
        rerank_memory_path=tmp_path / "rerank_memory.json",
        bias_memory_path=tmp_path / "bias_memory.json",
        concept_memory_path=tmp_path / "concept_memory.json",
        forecast_memory_path=tmp_path / "forecast_memory.json",
        long_term_plan_memory_path=tmp_path / "long_term_plan_memory.json",
        curriculum_memory_path=tmp_path / "curriculum_memory.json",
        review_memory_path=tmp_path / "review_memory.json",
        plan_memory_path=tmp_path / "plan_memory.json",
        strategic_memory_path=tmp_path / "strategic_memory.json",
        feature_memory_path=tmp_path / "feature_memory.json",
    )

    assert result.smoke_test_mode == "one_move_live_smoke_test_gate"
    assert result.selected_move_is_legal is True
    assert result.one_move_gate_passed is False
    assert result.live_lichess_send_enabled is False
    assert result.lichess_move_post_attempted is False
    assert result.send_blocked_by_smoke_gate is True


def test_phase22e15_consumes_preflight_and_guarded_gate(tmp_path: Path):
    result = run_full_chess_one_move_live_smoke_test_gate_kernel(
        memory_path=tmp_path / "smoke_memory.json",
        preflight_memory_path=tmp_path / "preflight_memory.json",
        gate_memory_path=tmp_path / "gate_memory.json",
        adapter_memory_path=tmp_path / "adapter_memory.json",
        loop_memory_path=tmp_path / "loop_memory.json",
        rerank_memory_path=tmp_path / "rerank_memory.json",
        bias_memory_path=tmp_path / "bias_memory.json",
        concept_memory_path=tmp_path / "concept_memory.json",
        forecast_memory_path=tmp_path / "forecast_memory.json",
        long_term_plan_memory_path=tmp_path / "long_term_plan_memory.json",
        curriculum_memory_path=tmp_path / "curriculum_memory.json",
        review_memory_path=tmp_path / "review_memory.json",
        plan_memory_path=tmp_path / "plan_memory.json",
        strategic_memory_path=tmp_path / "strategic_memory.json",
        feature_memory_path=tmp_path / "feature_memory.json",
        require_token_for_pass=False,
    )

    assert result.preflight_consumed is True
    assert result.guarded_gate_consumed is True
    assert result.preflight_trace_hash
    assert result.guarded_gate_trace_hash
    assert result.preflight_passed is True


def test_phase22e15_even_with_flags_no_post_in_this_phase(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("LICHESS_BOT_TOKEN", "test-token")

    result = run_full_chess_one_move_live_smoke_test_gate_kernel(
        memory_path=tmp_path / "smoke_memory.json",
        preflight_memory_path=tmp_path / "preflight_memory.json",
        gate_memory_path=tmp_path / "gate_memory.json",
        adapter_memory_path=tmp_path / "adapter_memory.json",
        loop_memory_path=tmp_path / "loop_memory.json",
        rerank_memory_path=tmp_path / "rerank_memory.json",
        bias_memory_path=tmp_path / "bias_memory.json",
        concept_memory_path=tmp_path / "concept_memory.json",
        forecast_memory_path=tmp_path / "forecast_memory.json",
        long_term_plan_memory_path=tmp_path / "long_term_plan_memory.json",
        curriculum_memory_path=tmp_path / "curriculum_memory.json",
        review_memory_path=tmp_path / "review_memory.json",
        plan_memory_path=tmp_path / "plan_memory.json",
        strategic_memory_path=tmp_path / "strategic_memory.json",
        feature_memory_path=tmp_path / "feature_memory.json",
        explicit_live_send_authorized=True,
        human_operator_confirmed=True,
        live_game_stream_confirmed=True,
        one_move_gate_enabled=True,
    )

    assert result.token_present is True
    assert result.explicit_live_send_authorized is True
    assert result.human_operator_confirmed is True
    assert result.live_game_stream_confirmed is True
    assert result.one_move_gate_enabled is True
    assert result.one_move_gate_passed is False
    assert result.live_lichess_send_enabled is False
    assert result.lichess_move_post_attempted is False


def test_phase22e15_memory_persists_across_runs(tmp_path: Path):
    memory_path = tmp_path / "smoke_memory.json"

    first = run_full_chess_one_move_live_smoke_test_gate_kernel(
        memory_path=memory_path,
        preflight_memory_path=tmp_path / "preflight_memory.json",
        gate_memory_path=tmp_path / "gate_memory.json",
        adapter_memory_path=tmp_path / "adapter_memory.json",
        loop_memory_path=tmp_path / "loop_memory.json",
        rerank_memory_path=tmp_path / "rerank_memory.json",
        bias_memory_path=tmp_path / "bias_memory.json",
        concept_memory_path=tmp_path / "concept_memory.json",
        forecast_memory_path=tmp_path / "forecast_memory.json",
        long_term_plan_memory_path=tmp_path / "long_term_plan_memory.json",
        curriculum_memory_path=tmp_path / "curriculum_memory.json",
        review_memory_path=tmp_path / "review_memory.json",
        plan_memory_path=tmp_path / "plan_memory.json",
        strategic_memory_path=tmp_path / "strategic_memory.json",
        feature_memory_path=tmp_path / "feature_memory.json",
        require_token_for_pass=False,
    )
    second = run_full_chess_one_move_live_smoke_test_gate_kernel(
        memory_path=memory_path,
        preflight_memory_path=tmp_path / "preflight_memory.json",
        gate_memory_path=tmp_path / "gate_memory.json",
        adapter_memory_path=tmp_path / "adapter_memory.json",
        loop_memory_path=tmp_path / "loop_memory.json",
        rerank_memory_path=tmp_path / "rerank_memory.json",
        bias_memory_path=tmp_path / "bias_memory.json",
        concept_memory_path=tmp_path / "concept_memory.json",
        forecast_memory_path=tmp_path / "forecast_memory.json",
        long_term_plan_memory_path=tmp_path / "long_term_plan_memory.json",
        curriculum_memory_path=tmp_path / "curriculum_memory.json",
        review_memory_path=tmp_path / "review_memory.json",
        plan_memory_path=tmp_path / "plan_memory.json",
        strategic_memory_path=tmp_path / "strategic_memory.json",
        feature_memory_path=tmp_path / "feature_memory.json",
        require_token_for_pass=False,
    )

    assert first.memory_loaded is False
    assert second.memory_loaded is True
    assert second.final_smoke_gate_policy["kernel_run_count"] == 2
    assert second.final_smoke_gate_policy["move_post_attempt_total"] == 0


def test_phase22e15_boundary_no_post_or_external_engines(tmp_path: Path):
    result = run_full_chess_one_move_live_smoke_test_gate_kernel(
        memory_path=tmp_path / "smoke_memory.json",
        preflight_memory_path=tmp_path / "preflight_memory.json",
        gate_memory_path=tmp_path / "gate_memory.json",
        adapter_memory_path=tmp_path / "adapter_memory.json",
        loop_memory_path=tmp_path / "loop_memory.json",
        rerank_memory_path=tmp_path / "rerank_memory.json",
        bias_memory_path=tmp_path / "bias_memory.json",
        concept_memory_path=tmp_path / "concept_memory.json",
        forecast_memory_path=tmp_path / "forecast_memory.json",
        long_term_plan_memory_path=tmp_path / "long_term_plan_memory.json",
        curriculum_memory_path=tmp_path / "curriculum_memory.json",
        review_memory_path=tmp_path / "review_memory.json",
        plan_memory_path=tmp_path / "plan_memory.json",
        strategic_memory_path=tmp_path / "strategic_memory.json",
        feature_memory_path=tmp_path / "feature_memory.json",
        require_token_for_pass=False,
    )

    assert result.evidence["no_move_post"] is True
    assert result.evidence["live_lichess_send_enabled"] is False
    assert result.evidence["lichess_move_post_attempted"] is False
    assert result.evidence["uses_stockfish"] is False
    assert result.evidence["uses_llm_shortcut"] is False
    assert result.evidence["uses_lichess_analysis"] is False
