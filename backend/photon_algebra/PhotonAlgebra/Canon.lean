...
def insertByKey (x : Expr) : List Expr → List Expr
  | [] => [x]
  | y :: ys =>
      if leStr x.key y.key then x :: y :: ys
      else y :: insertByKey x ys
...
def dedupSortedByKey (xs : List Expr) : List Expr :=
  let rec go (prev? : Option String) (acc : List Expr) (rest : List Expr) :=
    ...
    | x :: rs =>
        ...
        match prev? with
        | some pk =>
            if k == pk then go prev? acc rs else go (some k) (x :: acc) rs
        | none => go (some k) (x :: acc) rs
  go none [] xs
...
def absorb (terms : List Expr) : List Expr :=
  let bases := terms.filter (fun t =>
    match t with | Expr.times _ => false | _ => true
  )
  terms.filter (fun t =>
    match t with
    | Expr.times _ => !(bases.any (fun b => timesHasFactor t b))
    | _ => true
  )
...
/-- Forward decl via mutual: factor uses canonPlus and canonPlus uses factor. -/
mutual

/-- If all terms are `entangle a _` with same `a`, factor into `a ↔ canonPlus(rights)`. -/
def factorEntangleIfPossible (terms : List Expr) : Option Expr :=
  ...
  | (Expr.entangle a b) :: rest =>
      if rest.all (fun t =>
        ...
        some (Expr.entangle a (canonPlus rights))
      else
        none
  | _ => none
