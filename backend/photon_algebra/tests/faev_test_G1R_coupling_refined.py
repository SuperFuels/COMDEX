...
gamma = 0.002     # damping coefficient for ψ
delta_E = 0.001   # exponential normalization factor
...
# --- Evolution Loop ---
for i in range(1, T):
    # Hidden field ψ evolution (now includes damping term)
    dpsi = -alpha_psi * R[i-1] + β * np.sin(phi[i-1]) - gamma * dpsi_prev
    psi[i] = psi[i-1] + dpsi * dt
    dpsi_prev = dpsi

    # Curvatures with mutual coupling
    R[i] = np.sin(omega0 * t[i]) + alpha_psi * psi[i-1]
    R_psi[i] = np.sin(omega0 * t[i] + np.pi/4) - alpha_psi * phi[i-1]

    # Visible field φ feedback (with mild phase damping)
    dphi = -omega0**2 * phi[i-1] - β * phi[i-1]**3 + alpha_psi * psi[i-1] - 0.002 * phi[i-1]
    phi[i] = phi[i-1] + dphi * dt + np.random.normal(0, noise)

    # Unified energy (with normalization)
...
results = {
    ...
    "constants": {
        ...
        "gamma": gamma, "delta_E": delta_E, "noise": noise
    },
    "metrics": {
