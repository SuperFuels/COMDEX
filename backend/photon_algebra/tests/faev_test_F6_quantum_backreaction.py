...
#   to prevent singularity collapse and stabilize the cosmological bounce.
# ==========================================================
...
eta_Q = 0.015      # quantum backreaction gain
zeta_Q = 0.004     # vacuum-derivative feedback
xi_Q = 0.0008      # decoherence noise amplitude
eps = 1e-6         # avoid divide-by-zero in curvature terms
...
# --- Helper functions ---
def V(phi):
    """Softened potential to avoid unbounded growth."""
...
for i in range(1, T):
    ...
    rho_curv[i] = delta / (a[i-1] ** 2 + eps)
    rho_tot[i] = rho_m[i] + rho_phi[i] + rho_curv[i] + Lambda_eff[i-1]

    # Energy derivative for vacuum feedback
    if i > 2:
        dE = (rho_tot[i] - rho_tot[i-1]) / dt
    else:
        dE = 0.0

    # Quantum-modified Λ evolution
    Lambda_eff[i] = Lambda_eff[i-1] - zeta_Q * dE + np.random.normal(0, xi_Q)
    Lambda_eff[i] = np.clip(Lambda_eff[i], -0.01, 0.02)

    # Quantum backreaction correction
    if i > 2:
        dH = (H[i-1] - H[i-2]) / dt
    else:
        dH = 0.0
    Q_back = -eta_Q * (dH ** 2) / (abs(H[i-1]) + eps)

    # Update Hubble parameter
    H[i] = H[i-1] + dt * (-alpha * rho_m[i] + Lambda_eff[i] - xi * H[i-1] + Q_back)
...
# --- Classification ---
if a_min > 0.08 and energy_max < 200 and mean_coherence > 0.6:
    verdict = "✅ Stable Quantum Bounce (Backreaction Closure)"
elif a_min > 0.03 and mean_coherence > 0.4:
    verdict = "⚠️ Semi-Stable (partial quantum rebound)"
else:
...
results = {
    ...
    "files": {
        "scale_plot": "FAEV_F6_ScaleFactorEvolution.png",
        "energy_plot": "FAEV_F6_EnergyDecomposition.png",
        "phase_plot": "FAEV_F6_PhaseCoherence.png"
    },
