...
if not RPC_URL:
    raise RuntimeError("Missing WEB3_PROVIDER_URL environment variable")
...
if not escrow_addr_env:
    raise RuntimeError("Missing ESCROW_CONTRACT_ADDRESS environment variable")
try:
    ESCROW_ADDR = Web3.to_checksum_address(escrow_addr_env)
except ValueError:
    raise RuntimeError(f"Invalid ESCROW_CONTRACT_ADDRESS: {escrow_addr_env}")
...
if not deployer_pk:
    raise RuntimeError("Missing DEPLOYER_PRIVATE_KEY environment variable")
...
def release_deal(
    ...
    # Build, sign & send the on-chain release transaction
