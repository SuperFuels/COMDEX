...
def extract_all_glyphs(symbolic_tree: dict) -> list:
    """
    Recursively extract all glyphs from the symbolic tree, including nested expression glyphs.
    """
    extracted = []
    ...
    def recurse(node):
        ...
        if glyph:
            if glyph.get("type") == "expression" and "glyphs" in glyph:
                for subglyph in glyph["glyphs"]:
                    extracted.append(subglyph)
            else:
                extracted.append(glyph)

    for node in symbolic_tree.get("nodes", []):
        recurse(node)
