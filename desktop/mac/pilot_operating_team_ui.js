(function installPilotOperatingTeamUI() {
  ...
  function departmentPanel(department) {
    const key = department === "hr" ? "people" : department;
    const capsule = (runtime.workspace?.capsules || []).find((item) => item.department_id === key);
    if (!capsule) return "";
    const human = capsule.control_state === "human";
    return `
      <section class="pilot-department-computer" data-pilot-department-computer-panel>
        <div>
          <span>Department Computer</span>
          <strong>${human ? "Founder has control" : "Pilot has control"}</strong>
          <small>Isolated session · exact approvals · evidence receipts</small>
        </div>
        <button type="button" data-pilot-open-computer="${escapeHtml(key)}">Watch work</button>
        <button type="button" data-pilot-control="${escapeHtml(key)}" data-human="${human ? "false" : "true"}">
          ${human ? "Return control to Pilot" : "Take control"}
        </button>
      </section>
    `;
  }
  ...
  function render() {
    ...
    if (train) {
      if (!runtime.trainSurfacePresent) runtime.teamVisible = true;
    ...
    document.querySelectorAll("[data-aion-shared-department-pilot]").forEach((surface) => {
      const department = surface.getAttribute("data-aion-shared-department");
      const existing = surface.querySelector(":scope > [data-pilot-department-computer-panel]");
      const wrapper = document.createElement("div");
      wrapper.innerHTML = departmentPanel(department);
      const next = wrapper.firstElementChild;
      if (!next) return;
      if (!existing) surface.prepend(next);
